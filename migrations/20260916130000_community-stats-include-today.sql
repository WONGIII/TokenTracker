-- Community stats must include TODAY, like the leaderboard's all-time figure.
--
-- The builder unions the daily rollups (complete days) with a live tail read from
-- tokentracker_hourly, and it started that tail at the raw rollup watermark. The
-- leaderboard readers instead truncate the watermark to the start of its day
-- (their v_cut) so today is always recomputed from the hourly table. Without that
-- truncation, a watermark pushed forward into today — which happens whenever the
-- rollups are repaired — silently drops every hour before it: the panel reported
-- 6.92e9 against a real 7.45e9.
--
-- This replaces the upstream definition with the truncated window. Verified on
-- production: the snapshot's total_tokens now equals sum(total_tokens) from
-- tokentracker_hourly exactly.
...
BEGIN
  ...
  WITH usage_parts AS MATERIALIZED (
    ...
    FROM public.leaderboard_hourly_dedup(date_trunc('day', v_through AT TIME ZONE 'UTC') AT TIME ZONE 'UTC', v_to) d
  ),
