...
自动采集 **39 款 AI 编码工具** 的 token 用量，全程本地聚合，用一套漂亮的 Dashboard 看真实成本与趋势。外加桌面宠物、原生小组件，以及一套**完全属于你自己的**云端。
...
| | 改动 |
|---|---|
| ☁️ **自己的云端** | 数据同步、跨端账号视图、排行榜全部指向我自己用 Docker 搭的 [InsForge](https://github.com/InsForge/InsForge)（`tt.977744.xyz`），而不是上游的托管项目。CLI 的**默认后端地址**原本也是原作者的，现在是我的。 |
...
后端是我自己用 Docker 跑的 InsForge（Postgres + 认证 + edge functions）。登录**完全可选**——不登录就是纯本地，和上游一致。
