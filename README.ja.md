...
| 🐟 **DeepSeek Harness を独立集計** | 本家は `dsh` を "Other" に混ぜていました。ランキングは専用列（アイコン付き）、詳細モーダルは provider 内訳で独立表示、**セッション一覧**も収録し専用フィルタで絞れます（この環境では 392 件中 308 件）。 |
...
| 🔧 **一度も動いていなかった機能を修正** | 実績、プロフィールのいいね、ランキングの集計、コミュニティ統計はこのコードベースでは一度も動いていませんでした（テーブル欠落、関数欠落、スケジュール未設定、集計にデータなし）。すべて修正し migrations に記録しました。 |
