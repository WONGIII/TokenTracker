...
| 🐟 **DeepSeek Harness 별도 집계** | 원본은 `dsh`를 "Other"에 섞었습니다. 리더보드 전용 열(아이콘 포함), 상세 모달의 provider 내역, 그리고 **세션 목록**에서도 수집되어 전용 필터로 볼 수 있습니다(이 환경의 392개 중 308개). |
...
| 🔧 **한 번도 동작한 적 없던 기능 수정** | 업적, 프로필 좋아요, 리더보드 집계, 커뮤니티 통계는 이 코드베이스에서 한 번도 동작하지 않았습니다(테이블·함수 누락, 스케줄 없음, 집계 데이터 없음). 모두 수정하고 migrations에 기록했습니다. |
