...
| 🎮 **OpenBitFun 单独统计** | 同样自动找到数据目录（`~/.openbitfun`），每次调用的 token、模型、会话单独统计，排行榜单独一列（带官方图标），会话列表同样收录。注意：它的记录里**没有缓存 token 的拆分**，所以命中的输入会按未命中计价，成本偏高。路径特殊时用 `TOKENTRACKER_OPENBITFUN_DIR` 指过去。 |
